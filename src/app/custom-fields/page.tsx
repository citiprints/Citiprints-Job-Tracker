							...
							{editingId === f.id ? (
								...
								<div className="flex items-center justify-between">
									<div>
										...
										<div className="text-xs text-gray-600">{f.type}{f.required ? " • required" : ""}</div>
									</div>
									<div className="flex items-center gap-2 text-xs">
										...
										<button
											...
											onClick={() => { setEditingId(f.id); setELabel(f.label); setEType(f.type); setERequired(f.required); setEOrder(f.order); }}
										>
											Edit
										</button>
										<button
